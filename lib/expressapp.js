...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.delete('/v2/pushnotifications/subscriptions/:token', function(req, res) {
    ...
    /* AIB Token Support */
  router.get('/v1/token/import/:txid', function (req, res) {
    //getServerWithAuth(req, res, function (server) {
      var server = getServer(req,res);
      req.body.txid = req.params['txid'];
      server.importToken(req.body, function (err, response) {
        if (err) return returnError(err, res, req);
        res.json(response);
      });
  });

  router.delete('/v1/token/remove/:txid', function (req, res) {
    //getServerWithAuth(req, res, function (server) {
    getServer(req,res, function(server) {
      req.body.txid = req.params['txid'];
      server.removeToken(req.body, function (err) {
        if (err) return returnError(err, res, req);
          res.json({
            success: true
          });
  ...
  router.get('/v1/tokens/:limit', function (req, res) {
    //getServerWithAuth(req, res, function (server) {
    var server = getServer(req,res);
    server.getTokens(parseInt(req.params['limit'],10), function (err, tokens) {
      if (err) return returnError(err, res, req);
        res.json(tokens);
    });
  });
