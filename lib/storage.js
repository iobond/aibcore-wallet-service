...
Storage.prototype._createIndexes = function() {
  ...
  this.db.collection(collections.TOKENS).createIndex({
    id: 1,
  });
};
...
Storage.prototype.fetchToken = function(txid,cb) {
    this.db.collection(collections.TOKENS).findOne({ txid: txid,
    }, {
        limit: limit || 20
...
Storage.prototype.fetchTokens = function(limit,cb) {
    this.db.collection(collections.TOKENS).find({
    }, {
        limit: limit || 20
    }).toArray(function(err, result) {
        if (err) return cb(err);
        if (!result) return cb();
        var tks = _.map(result, function(tx) {
            return Model.Token.fromObj(tx);
        });
        return cb(null, tks);
    });
};
...
Storage.prototype.fetchToken = function(txid, cb) {
    this.db.collection(collections.TOKENS).findOne({
        txid: txid
    }, function(err, result) {
        if (err) return cb(err);
        if (!result) return cb(null,null);
        return cb(null, Model.Token.fromObj(result));
    });
};

Storage.prototype.storeToken = function(token, cb) {
    this.db.collection(collections.TOKENS).update({
        id: token.id
    }, token.toObject(), {
        w: 1,
        upsert: true,
