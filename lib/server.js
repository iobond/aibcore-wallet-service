          ...
          if (token) {
              bc.verifyTokenOrigin(token.txid, tx.txid, function (err, res) {
                  ...
                  if (res) {
                      ...
                      var tokenBalance;
                      var tokenUtxos = _.filter(utxos, {
                          txid: tx.txid,
                      });
                      tokenBalance = {
                          symbol: token.symbol,
                          tokenId: token.id,
                          totalAmount: _.sum(tokenUtxos, 'satoshis') * token.factor,
                          lockedAmount: _.sum(_.filter(tokenUtxos, 'locked'), 'satoshis') * token.factor,
                          totalConfirmedAmount: _.sum(_.filter(tokenUtxos, 'confirmations'), 'satoshis') * token.factor,
                          lockedConfirmedAmount: _.sum(_.filter(_.filter(tokenUtxos, 'locked'), 'confirmations'), 'satoshis') * token.factor,
                      };
