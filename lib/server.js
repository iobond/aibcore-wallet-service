 ...
 */
WalletService.prototype._totalizeTokenUtxos = function(utxos, txs) {
  var tokenList = [
      {
          txid: "021488ada59300fa746c532fbc7945b84296501c82acebe53643b64e8ba1fe51",
          layerMarker : "tl",
          version: 1,
          symbol: "AIBT",
          factor: 10,
  ...
  for ( var token in tokensUtxos ) {
    var tokenBalance = {
      symbol: token,
      totalAmount: _.sum(tokensUtxos[token], 'satoshis'),
      lockedAmount: _.sum(_.filter(tokensUtxos[token], 'locked'), 'satoshis'),
      totalConfirmedAmount: _.sum(_.filter(tokensUtxos[token], 'confirmations'), 'satoshis'),
      lockedConfirmedAmount: _.sum(_.filter(_.filter(tokensUtxos[token], 'locked'), 'confirmations'), 'satoshis'),
    };
...
WalletService.prototype._getBalanceFromAddresses = function(addresses, cb) {
  ...
  self.getTxHistory({}, function(err, res) {
      ...
      self._getUtxosForCurrentWallet(addresses, function (err, utxos) {
          ...
          balance.tokens = self._totalizeTokenUtxos(utxos, txs);
 ...
 * @param {Object} N/A
 */
WalletService.prototype.getTokens = function(cb) {
    var self = this;
...
WalletService.prototype.importToken = function(opts, cb) {
    ...
    var bc = this._getBlockchainExplorer();
    bc.getTransaction(opts.txid, function(err, tx) {
        if (err) return cb(err);
        return cb(null, !!tx);
    })
};
