 ...
 */
WalletService.prototype._totalizeTokenUtxos = function(utxos, txs,tokenList) {
  var tokenBalances = [];
  var tokensUtxos = {};

  tokenList.forEach(function (token) {
    tokensUtxos[token.symbol] = [];
    txs.forEach(function (tx) {
    var tokenOutputs = _.filter(tx.outputs, {
      payload: token.hex,
    });
    if (tokenOutputs.length > 0) {
      /* TODO:
      if (!verifytokenorigin {tokenid = token.txid, txid = utxo.txid} ) {
         console.log("txid of unspent isn't from the token origin");
       } else {
      */
      ...
      if (tokenUtxos.length > 0) {
        tokensUtxos[token.symbol].push(tokenUtxos[0]);
      } else {
        console.log("Invalid token utxos size found!\n");
      }
      }
    });
  });

  for (var token in tokensUtxos) {
    var tokenData = _.filter(tokenList, {symbol: token})[0];

    var tokenBalance = {
        symbol: token,
        totalAmount: _.sum(tokensUtxos[token], 'satoshis') * tokenData.factor,
        lockedAmount: _.sum(_.filter(tokensUtxos[token], 'locked'), 'satoshis') * tokenData.factor,
        totalConfirmedAmount:  _.sum(_.filter(tokensUtxos[token], 'confirmations'), 'satoshis') * tokenData.factor,
        lockedConfirmedAmount: _.sum(_.filter(_.filter(tokensUtxos[token], 'locked'), 'confirmations'), 'satoshis') * tokenData.factor,
    };
  ...
  self.getTxHistory({}, function(err, res) {
      ...
      self._getUtxosForCurrentWallet(addresses, function (err, utxos) {
          ...
          self.storage.fetchTokens(20, function (err,tokenList) {
              balance.tokens = self._totalizeTokenUtxos(utxos, txs, tokenList);
          ...
          function(next) {
              ...
              self.storage.fetchToken(opts.outputs[0].tokenId,function (err,token) {
                  ...
                  if (token)
                    var dataOutput;
                    dataOutput.amount = 0;
                    dataOutput.script = token.hex;
                    opts.outputs.push(dataOutput);
                  next();
              });
          },
          function(next) {
