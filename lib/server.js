 ...
 * @returns {Array} utxos - List of UTXOs.
 *
 * use token list as criteria to find all transactions with token signature and also use the txid
 * to group coresponding utxos with token
 */
WalletService.prototype._totalizeTokenUtxos = function(utxos, txs,tokenList,totalCb) {
  var tokenBalances = [];
  var tokenTxOutputs = {};
  var tokenTxs = [];
  txs.forEach(function (tx) {
      var tokenOutputs = _.filter(tx.outputs, function (output) {
          return !!output.payload;
      });
      if (tokenOutputs.length > 0) {
          tokenTxOutputs[tx.txid] = tokenOutputs[0];
          tokenTxs.push(tx);
      }
  });
  ...
  var network = 'livenet'; // Bitcore.Address(txs[0].address).toObject().network;
  var bc = this._getBlockchainExplorer(network);

  // TODO: performing testcase for possible multiple tx.outputs for different tokens
  var tokensUtxos = {};
  async.eachSeries(
      tokenTxs,
      function(tx,cb) {
          var tokenOutput = tokenTxOutputs[tx.txid];
          var token = _.filter(tokenList,function (token) {
            var payload = tokenOutput.payload;
            if (payload.substring(0,2) == '74')
              payload = payload.substring(2);
            return token.hex.substring(2) === payload;
          })[0];

          if (token) {
              bc.verifyTokenOrigin(token.txid, tx.txid, function (err, res) {
                  if (err) log.debug("unable to verify token origin of ", tx.id);
                  if (res) {
                      var tokenId = token.id;
                      var tokenUtxo = _.filter(utxos, {
                          txid: tx.txid,
                      })[0];
                      if (tokenUtxo) {
                          tokenUtxo.symbol = token.symbol;
                          tokenUtxo.tokenId = token.id;
                          tokenUtxo.factor = token.factor;

                          if (!tokensUtxos[tokenId]) {
                              var tokenUtxos = [];
                              tokensUtxos[tokenId] = tokenUtxos;
                          }
                          tokensUtxos[tokenId].push(tokenUtxo);
                      }
                  }
                  return cb();
              });
      ...
      function(err) {
          if (err) {
              log.debug("Token utxo sum: ",err);
              return (err,null);
          } else {
              for (var tokenId in tokensUtxos) {
                  var tokenUtxos = tokensUtxos[tokenId];
                  var factor = tokenUtxos[0].factor;
                  var symbol = tokenUtxos[0].symbol;
                  var tokenBalance = {
                      symbol: symbol,
                      tokenId: tokenId,
                      totalAmount: _.sum(tokenUtxos, 'satoshis') * factor,
                      lockedAmount: _.sum(_.filter(tokenUtxos, 'locked'), 'satoshis') * factor,
                      totalConfirmedAmount: _.sum(_.filter(tokenUtxos, 'confirmations'), 'satoshis') * factor,
                      lockedConfirmedAmount: _.sum(_.filter(_.filter(tokenUtxos, 'locked'), 'confirmations'), 'satoshis') * factor,
                  };
                  tokenBalances.push(tokenBalance);
              };
              return totalCb(null, tokenBalances);
          }
      }
  )
};
...
WalletService.prototype._getBalanceFromAddresses = function(addresses, cb) {
  ...
  self.getTxHistory({}, function(err, res) {
      if (err) return cb(err);
      txs = _.uniq(res, 'txid');
      ...
      self._getUtxosForCurrentWallet(addresses, function (err, utxos) {
          ...
          self.storage.fetchTokens(20, function (err,tokenList) {
              self._totalizeTokenUtxos(utxos, txs, tokenList, function (err,tokensBalance) {
                if (err) return cb(err,null);

                balance.tokens = tokensBalance;
    ...
    self.getWallet({}, function(err, wallet) {
      ...
      checkTxpAlreadyExists(opts.txProposalId, function(err, txp) {
        ...
        async.series([
          ...
          function(next) {
              // TODO: check for outputs more than one test case
              self.storage.fetchToken(opts.outputs[0].tokenId,function (err,token) {
                  if (err) return cb(err);
                  /*if (token.factor>0)
                    opts.outputs.forEach(function (output) {
                      output.amount = output.amount / token.factor;
                    });*/
                  if (token)
                    opts.nullData = token.hex.substring(2);
                  next();
              });
          },
          function(next) {
...
WalletService.prototype._normalizeTxHistory = function(txs) {
  ...
  return _.map(txs, function(tx) {
    ...
    var outputs = _.map(tx.vout, function(item) {
      ...
      var opcode = item.scriptPubKey.hex.substr(0,2);
      var data = item.scriptPubKey.hex.substr(4);
      /* OP_RETURN = 0x6a 1 byte after will be ignored*/

      return {
        address: itemAddr,
        amount: parseInt((item.value * 1e8).toFixed(0)),
        payload: opcode === '6a'? data : undefined,
      }
    });
    ...
    return _.map(txs, function(tx) {
      ...
      if (opts.includeExtendedInfo) {
        newTx.inputs = _.map(inputs, function(input) {
          return _.pick(input, 'address', 'amount', 'isMine','payload');
        });
        newTx.outputs = _.map(outputs, function(output) {
          return _.pick(output, 'address', 'amount', 'isMine','payload');
        });
      } else {
        var withPayload = outputs.filter(function(output){
        ...
        outputs = outputs.concat(withPayload);
 ...
 * @param {Object} N/A
 */
WalletService.prototype.getTokens = function(limit,cb) {
    var self = this;
    self.storage.fetchTokens(limit,function (err,tokens) {
        if (err) return cb(err,null);
 ...
 * @param {Object} N/A
 */
WalletService.prototype.removeToken = function(cb) {
    var self = this;
    self.storage.fetchTokens(cb)
};

/**
 * import and store a new token from a txid.
 * @param {Object} opts
 * @param {string} opts.txId - The tx id.
 * @returns {Object} token data
 */
WalletService.prototype._importToken = function(txid, cb) {
    var self = this;

    var bc = self._getBlockchainExplorer();
    bc.getTransaction(txid, function(err, tx) {
        if (err) return cb(err);
        var txs = [];
        txs.push(tx);
        txs = self._normalizeTxHistory(txs);
        if (txs.length == 0 ){
            console.log("normalize tx failed\n");
        } else if ( txs.length >1 ) {
            console.log("only one transaction expected. Check error\n");
        }
        var normalizedTx = txs.pop();

        var inputAddrs = normalizedTx.inputs.map(function(input) {
            return input.address;
        });

        var tokenData = {};
        /* parse tx for payload and deposit */
        normalizedTx.outputs.forEach(function(output){
            if (!output.address && output.amount == 0) {
                tokenData.payload = output.payload;
            } else {
                /* if output address not found in input addresses, found the deposit address */
                if (inputAddrs.indexOf(output.address) == -1) {
                    tokenData.address = output.address;
                    tokenData.deposit = output.amount;
                }
            }
        });
        ...
        var tokenModel;
        if (tokenData) {
            var token = new Bitcore.Token().fromHexString(tokenData.payload);
            tokenModel = Token.create({
                address: tokenData.address,
                deposit: tokenData.deposit,
                hex:  tokenData.payload,
                txid: txid,
                symbol: token.symbol,
                factor: token.factor,
                fullName: token.fullName,
            });
            self.storage.storeToken(tokenModel, function(err) {
                log.debug('Token created', token.id, token.symbol);
                if (err) return cb(err,tokenModel);
            });
        }
        return cb(null, tokenModel);
    })
};
/**
 * import a token from a txid.
 * @param {Object} opts
 * @param {string} opts.txId - The tx id.
 * @returns {Object} token data
 */
WalletService.prototype.importToken = function(opts, cb) {
  var self = this;
  self.storage.fetchToken(opts.txid,function(err,tokenExisted){
     if (err) {
         console.log("db data retreiving error\n");
         return cb();
     }
     if (tokenExisted) {
       var response = {};
       response["existed"]  = true;
       response["matched"] = tokenExisted;
       return cb(null, tokenExisted);
     } else {
       self._importToken(opts.txid,function (err,tokenStored) {
          if (err) return cb(err,null);
          return cb(null,tokenStored);
       });
     }
  });
};
