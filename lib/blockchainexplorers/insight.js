...
Insight.prototype.verifyTokenOrigin = function(tokenid,txid, cb) {
    var args = {
        method: 'POST',
        path: this.apiPrefix + '/verifytokenorigin',
        json: {
            tokenid: tokenid,
            txid: txid,
        },
    ...
    this._doRequest(args, function(err, res) {
        if (err || res.statusCode !== 200 ) return cb(_parseErr(err, res));

        var isTokenOrigin = false;
        if (_.isObject(res)) {
          if (res.error) return cb(_parseErr(res.error, res));
          isTokenOrigin = res.body.result;
        }
        return cb(null,isTokenOrigin);
    });
};
