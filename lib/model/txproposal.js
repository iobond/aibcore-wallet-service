...
TxProposal.create = function(opts) {
  ...
  x.outputOrder = _.range(x.outputs.length + 1 + dataOutputCnt);
...
TxProposal.prototype._buildTx = function() {
  ...
  // add data OP_RETURN
  if (self.nullData) 
    if (_.isString(self.nullData)) { //  && JSUtil.isHexa(self.nullData)) {
      t.addData(new buffer.Buffer(self.nullData, 'hex'));
    }
